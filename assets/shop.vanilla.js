...
function updateLayoutFromSettings() {
  ...
  if (isListActive) {
    ...
    if (newDefaultGridLayout) {
      const defaultLayoutBtn = document.querySelector(
        `.tf-view-layout-switch[data-value-layout="${newDefaultGridLayout}"]`
      );
...
function initProductFilters() {
  ...
  document.querySelectorAll('input[name="availability"]').forEach((input) => {
    input.addEventListener('change', function () {
      ...
      const checkedAvailability = Array.from(document.querySelectorAll('input[name="availability"]:checked')).map(
        (input) =>
          input.id === 'inStock'
            ? window.translations?.shop?.in_stock || 'In stock'
            : window.translations?.shop?.out_of_stock || 'Out of stock'
      );
  ...
  document.querySelectorAll('input[name="brand"]').forEach((input) => {
    input.addEventListener('change', function () {
      ...
      const checkedBrands = Array.from(document.querySelectorAll('input[name="brand"]:checked')).map(
        (input) => input.value
      );
...
function handleFilterResponse(html, filterUrl) {
  try {
    ...
    // Trigger error event
    document.dispatchEvent(
      new CustomEvent('filterParseError', {
...
function updateProductCount(count) {
  ...
  const countElements = document.querySelectorAll(
    '#product-count-grid, #product-count-list, .product-count, .total-products'
  );
  countElements.forEach((el) => {
    if (
      el.textContent.includes(window.translations?.shop?.products_found || 'Products found') ||
      el.textContent.includes(window.translations?.shop?.products || 'products')
    ) {
  ...
  const noProductsMessage = `
            <div class="no-products-found">
                <div class="no-products-content">
                    ...
                    <p>${
                      window.translations?.shop?.try_adjusting_filters ||
                      'Try adjusting your filters or search criteria.'
                    }</p>
                    <button class="btn-reset-filters" onclick="resetAllFilters()">${
                      window.translations?.shop?.reset_all_filters || 'Reset All Filters'
                    }</button>
                </div>
            </div>
        `;
...
function initializeFiltersFromURL() {
  ...
  if (brandFilter) {
    ...
    // Update UI to show active state
    document.querySelectorAll('input[name="brand"]').forEach((input) => {
      if (input.value === brandFilter) {
        // Compare with value instead of id
        input.checked = true;
      }
    });
  }
...
function updateMetaFilter() {
  ...
  if (filters.availability) {
    tags.push(
      `<span class="filter-tag"><span class="remove-tag icon-close" data-filter="availability"></span> ${
        window.translations?.shop?.availability || 'Availability'
      }: ${filters.availability}</span>`
    );
  }
  if (filters.brands) {
    tags.push(
      `<span class="filter-tag"><span class="remove-tag icon-close" data-filter="brands"></span> ${
        window.translations?.shop?.brand || 'Brand'
      }: ${filters.brands}</span>`
    );
  }
  ...
  if (priceSlider) {
    ...
    if (filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
      ...
      tags.push(
        `<span class="filter-tag"><span class="remove-tag icon-close" data-filter="price"></span> ${
          window.translations?.shop?.price || 'Price'
        }: $${displayMinPrice} - $${displayMaxPrice}</span>`
      );
    }
  }
  ...
  if (filters.color) {
    tags.push(
      `<span class="filter-tag"><span class="remove-tag icon-close" data-filter="color"></span> ${
        window.translations?.shop?.color || 'Color'
      }: ${filters.color}</span>`
    );
  }
  if (filters.size) {
    tags.push(
      `<span class="filter-tag"><span class="remove-tag icon-close" data-filter="size"></span> ${
        window.translations?.shop?.size || 'Size'
      }: ${filters.size}</span>`
    );
  }
  if (filters.sale) {
    tags.push(
      `<span class="filter-tag on-sale">${
        window.translations?.shop?.on_sale || 'On Sale'
      } <span class="remove-tag icon-close" data-filter="sale"></span></span>`
    );
  }
...
function updateLastVisibleItem() {
  setTimeout(() => {
    ...
    const lastVisible = Array.from(document.querySelectorAll('.card-product.style-list'))
...
function initSortFunctionality() {
  ...
  let originalProductsList = Array.from(document.querySelectorAll('#listLayout .card-product')).map((p) =>
    p.cloneNode(true)
  );
  let originalProductsGrid = Array.from(document.querySelectorAll('#gridLayout .card-product')).map((p) =>
    p.cloneNode(true)
  );
  ...
  document.querySelectorAll('.select-item').forEach((item) => {
    item.addEventListener('click', function () {
      ...
      // Only update sort dropdown, not variant dropdowns
      const sortDropdown = this.closest('.tf-dropdown-sort');
      if (sortDropdown) {
        const textSortValue = sortDropdown.querySelector('.text-sort-value');
        if (textSortValue) {
...
function initLayoutSwitching() {
  ...
  if (window.collectionData && window.collectionData.defaultLayoutType === 'list') {
    ...
    if (window.collectionData && window.collectionData.defaultGridLayout) {
      const defaultLayoutBtn = document.querySelector(
        `.tf-view-layout-switch[data-value-layout="${window.collectionData.defaultGridLayout}"]`
      );
      if (defaultLayoutBtn) {
      ...
      const defaultGridBtn =
        document.querySelector('.tf-view-layout-switch[data-value-layout="tf-col-3"]') ||
...
function initLoadMore() {
  ...
  if (loadMoreListBtn) {
    loadMoreListBtn.addEventListener('click', function () {
      listItemsDisplayed = showMoreItems(document.getElementById('listLayout'), listItemsPerPage, listItemsDisplayed);
    });
  }
  ...
  if (loadMoreGridBtn) {
    loadMoreGridBtn.addEventListener('click', function () {
      gridItemsDisplayed = showMoreItems(document.getElementById('gridLayout'), gridItemsPerPage, gridItemsDisplayed);
    });
  }
...
function handleLoadMore(layout) {
  const button =
    layout === 'list' ? document.getElementById('loadMoreListBtn') : document.getElementById('loadMoreGridBtn');
...
function hasActiveFilters() {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const hasUrlFilters =
    urlParams.has('filter.v.option1') ||
